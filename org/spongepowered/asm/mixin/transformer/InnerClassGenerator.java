...
final class InnerClassGenerator implements IClassGenerator {
    ...
    private byte[] generate(InnerClassGenerator.InnerClassInfo info) {
        try {
            ...
            ClassReader ex = new ClassReader(info.getClassBytes());
            MixinClassWriter cw = new MixinClassWriter(ex, 0);

            ex.accept(new InnerClassGenerator.InnerClassAdapter(cw, info), 8);
            return cw.toByteArray();
        } catch (InvalidMixinException invalidmixinexception) {
            throw invalidmixinexception;
        } catch (Exception exception) {
            InnerClassGenerator.logger.catching(exception);
            return null;
        }
    }
    ...
    static class InnerClassInfo extends Remapper {
        ...
        public String map(String key) {
            return this.originalName.equals(key) ? this.name : (this.ownerName.equals(key) ? this.targetName : key);
        }
