...
public class ExtensionClassExporter implements IExtension {
    ...
    public ExtensionClassExporter(MixinEnvironment env) {
        ...
        try {
            ...
            ExtensionClassExporter.logger.warn("Error cleaning class output directory: {}", new Object[] { ioexception.getMessage()});
        }
    ...
    private IDecompiler initDecompiler(MixinEnvironment env, File outputPath) {
        if (!env.getOption(MixinEnvironment.Option.DEBUG_EXPORT_DECOMPILE)) {
            ...
            try {
                boolean th = env.getOption(MixinEnvironment.Option.DEBUG_EXPORT_DECOMPILE_THREADED);

                ExtensionClassExporter.logger.info("Attempting to load Fernflower decompiler{}", new Object[] { th ? " (Threaded mode)" : ""});
                String className = "org.spongepowered.asm.mixin.transformer.debug.RuntimeDecompiler" + (th ? "Async" : "");
                Class clazz = Class.forName(className);
                Constructor ctor = clazz.getDeclaredConstructor(new Class[] { File.class});
                IDecompiler decompiler = (IDecompiler) ctor.newInstance(new Object[] { outputPath});

                ExtensionClassExporter.logger.info("Fernflower decompiler was successfully initialised, exported classes will be decompiled{}", new Object[] { th ? " in a separate thread" : ""});
                return decompiler;
            } catch (Throwable throwable) {
                ExtensionClassExporter.logger.info("Fernflower could not be loaded, exported classes will not be decompiled. {}: {}", new Object[] { throwable.getClass().getSimpleName(), throwable.getMessage()});
                return null;
            }
        }
    }
