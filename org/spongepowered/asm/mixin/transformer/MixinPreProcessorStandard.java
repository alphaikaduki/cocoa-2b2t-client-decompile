...
class MixinPreProcessorStandard {
    ...
    final MixinPreProcessorStandard prepare() {
        if (this.prepared) {
            ...
            Iterator iterator = this.classNode.mixinMethods.iterator();

            while (iterator.hasNext()) {
                MixinInfo.MixinMethodNode mixinField = (MixinInfo.MixinMethodNode) iterator.next();
                ClassInfo.Method method = this.mixin.getClassInfo().findMethod((MethodNode) mixinField);

                this.prepareMethod(mixinField, method);
            }

            iterator = this.classNode.fields.iterator();

            while (iterator.hasNext()) {
                FieldNode mixinField1 = (FieldNode) iterator.next();

                this.prepareField(mixinField1);
            }
    ...
    protected void prepareShadow(MixinInfo.MixinMethodNode mixinMethod, ClassInfo.Method method) {
        ...
        if (shadowAnnotation != null) {
            ...
            if (mixinMethod.name.startsWith(prefix)) {
                Annotations.setVisible((MethodNode) mixinMethod, MixinRenamed.class, new Object[] { "originalName", mixinMethod.name});
    ...
    protected void prepareSoftImplements(MixinInfo.MixinMethodNode mixinMethod, ClassInfo.Method method) {
        Iterator iterator = this.mixin.getSoftImplements().iterator();

        while (iterator.hasNext()) {
            InterfaceInfo iface = (InterfaceInfo) iterator.next();
    ...
    final MixinPreProcessorStandard conform(ClassInfo target) {
        ...
        while (iterator.hasNext()) {
            MixinInfo.MixinMethodNode mixinMethod = (MixinInfo.MixinMethodNode) iterator.next();
    ...
    protected boolean attachSpecialMethod(MixinTargetContext context, MixinInfo.MixinMethodNode mixinMethod, MixinPreProcessorStandard.SpecialMethod type) {
        ...
        if (annotation == null) {
            ...
            if (target == null) {
                ...
                if (target == null) {
                    throw new InvalidMixinException(this.mixin, String.format("%s method %s in %s was not located in the target class %s. %s%s", new Object[] { type, mixinMethod.name, this.mixin, context.getTarget(), context.getReferenceMapper().getStatus(), getDynamicInfo((MethodNode) mixinMethod)}));
                }
            ...
            } else if (!Bytecode.compareFlags((MethodNode) mixinMethod, target, 8)) {
        ...
        if (method != null && (method.isUnique() || this.mixin.isUnique() || method.isSynthetic())) {
            ...
            if (target == null) {
                ...
                if (Bytecode.getVisibility((MethodNode) mixinMethod).ordinal() < Bytecode.Visibility.PUBLIC.ordinal()) {
                    String unique1 = context.getUniqueName(mixinMethod, false);

                    MixinPreProcessorStandard.logger.log(this.mixin.getLoggingLevel(), "Renaming {} method {}{} to {} in {}", new Object[] { type, mixinMethod.name, mixinMethod.desc, unique1, this.mixin});
                    mixinMethod.name = method.renameTo(unique1);
                    return false;
                } else if (this.strictUnique) {
                    throw new InvalidMixinException(this.mixin, String.format("Method conflict, %s method %s in %s cannot overwrite %s%s in %s", new Object[] { type, mixinMethod.name, this.mixin, target.name, target.desc, context.getTarget()}));
                } else {
                    AnnotationNode unique = Annotations.getVisible((MethodNode) mixinMethod, Unique.class);

                    if (unique != null && ((Boolean) Annotations.getValue(unique, "silent", (Object) Boolean.FALSE)).booleanValue()) {
                        context.addMixinMethod(mixinMethod);
                        return true;
                        ...
                        try {
                            ...
                            if (this.verboseLogging || this.env.getOption(MixinEnvironment.Option.DEBUG_VERIFY)) {
                                syntheticbridgeexception.printAnalysis(context, target, mixinMethod);
                            }
        ...
        while (iter.hasNext()) {
            ...
            if (!this.validateField(context, mixinField, shadow)) {
                ...
                if (target == null) {
                    ...
                    if (target == null) {
                        throw new InvalidMixinException(this.mixin, String.format("Shadow field %s was not located in the target class %s. %s%s", new Object[] { mixinField.name, context.getTarget(), context.getReferenceMapper().getStatus(), getDynamicInfo(mixinField)}));
                    }
                ...
                if (field.isUnique()) {
                    if ((mixinField.access & 6) != 0) {
                        String isFinal = context.getUniqueName(mixinField);

                        MixinPreProcessorStandard.logger.log(this.mixin.getLoggingLevel(), "Renaming @Unique field {}{} to {} in {}", new Object[] { mixinField.name, mixinField.desc, isFinal, this.mixin});
                        mixinField.name = field.renameTo(isFinal);
                    } else {
                        if (this.strictUnique) {
                            throw new InvalidMixinException(this.mixin, String.format("Field conflict, @Unique field %s in %s cannot overwrite %s%s in %s", new Object[] { mixinField.name, this.mixin, target.name, target.desc, context.getTarget()}));
                        }
                    ...
                    iter.remove();
                    if (isShadow) {
                        boolean isFinal1 = field.isDecoratedFinal();

                        if (this.verboseLogging && Bytecode.hasFlag(target, 16) != isFinal1) {
                            String message = isFinal1 ? "@Shadow field {}::{} is decorated with @Final but target is not final" : "@Shadow target {}::{} is final but shadow is not decorated with @Final";
        ...
        if (Bytecode.hasFlag(field, 8) && !Bytecode.hasFlag(field, 2) && !Bytecode.hasFlag(field, 4096) && shadow == null) {
            ...
            if (field.name.startsWith(prefix)) {
                ...
                if (field.access != 2) {
                    ...
                    throw new InvalidMixinException(this.mixin, String.format("Imaginary super field %s.%s must have the same type as the parent mixin (%s)", new Object[] { context, field.name, this.mixin.getClassName()}));
                } else {
                    return false;
                }
            } else {
    ...
    protected void transform(MixinTargetContext context) {
        ...
        while (iterator.hasNext()) {
            ...
            ListIterator iter = mixinMethod.instructions.iterator();
    ...
    static enum SpecialMethod {
        ...
        MERGE(true), OVERWRITE(true, Overwrite.class), SHADOW(false, Shadow.class), ACCESSOR(false, Accessor.class), INVOKER(false, Invoker.class);
