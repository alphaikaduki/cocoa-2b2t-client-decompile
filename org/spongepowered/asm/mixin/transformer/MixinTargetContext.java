...
public class MixinTargetContext extends ClassContext implements IMixinContext {
    ...
    MixinTargetContext(MixinInfo mixin, ClassNode classNode, TargetClassContext context) {
        ...
        Iterator iterator = this.mixin.getInnerClasses().iterator();

        while (iterator.hasNext()) {
            String innerClass = (String) iterator.next();
    ...
    void addMixinMethod(MethodNode method) {
        Annotations.setVisible(method, MixinMerged.class, new Object[] { "mixin", this.getClassName()});
    ...
    void methodMerged(MethodNode method) {
        ...
        Annotations.setVisible(method, MixinMerged.class, new Object[] { "mixin", this.getClassName(), "priority", Integer.valueOf(this.getPriority()), "sessionId", this.sessionId});
    }
    ...
    public void transformMethod(MethodNode method) {
        ...
        for (ListIterator iter = method.instructions.iterator(); iter.hasNext(); lastInsn = insn) {
    ...
    private void transformLVT(MethodNode method) {
        if (method.localVariables != null) {
            Iterator iterator = method.localVariables.iterator();
    ...
    private void transformFieldRef(MethodNode method, Iterator iter, MemberRef fieldRef) {
        ...
        if (fieldRef.getOwner().equals(this.getClassRef())) {
            ...
            ClassInfo.Field fieldOwner = this.getClassInfo().findField(fieldRef.getName(), fieldRef.getDesc(), 10);

            if (fieldOwner != null && fieldOwner.isRenamed() && fieldOwner.getOriginalName().equals(fieldRef.getName()) && fieldOwner.isStatic()) {
                fieldRef.setName(fieldOwner.getName());
            }
        } else {
            ClassInfo fieldOwner1 = ClassInfo.forName(fieldRef.getOwner());

            if (fieldOwner1.isMixin()) {
                ClassInfo actualOwner = this.targetClassInfo.findCorrespondingType(fieldOwner1);
    ...
    private void checkFinal(MethodNode method, Iterator iter, FieldInsnNode fieldNode) {
        if (fieldNode.owner.equals(this.getTarget().getClassRef())) {
            ...
            if (opcode != 180 && opcode != 178) {
                Iterator iterator = this.shadowFields.entrySet().iterator();
                ...
                do {
                    ...
                    shadow = (Entry) iterator.next();
                    shadowFieldNode = (FieldNode) shadow.getKey();
    ...
    private void processImaginarySuper(MethodNode method, FieldInsnNode fieldInsn) {
        if (fieldInsn.getOpcode() != 180) {
            ...
            if (Annotations.getInvisible(method, SoftOverride.class) == null) {
                ...
                ListIterator methodIter = method.instructions.iterator(method.instructions.indexOf(fieldInsn));
    ...
    private String transformMethodDescriptor(String desc) {
        ...
        newDesc.append('(');
        Type[] atype = Type.getArgumentTypes(desc);
        int i = atype.length;
    ...
    public void postApply(String transformedName, ClassNode targetClass) {
        try {
            ...
            InjectorGroupInfo group = injectionvalidationexception.getGroup();

            throw new InjectionError(String.format("Critical injection failure: Callback group %s in %s failed injection check: %s", new Object[] { group, this.mixin, injectionvalidationexception.getMessage()}));
        }
    ...
    public void prepareInjections() {
        ...
        Iterator iterator = this.mergedMethods.iterator();
    ...
    public void applyInjections() {
        Iterator iterator = this.injectors.iterator();

        InjectionInfo injectInfo;

        while (iterator.hasNext()) {
            injectInfo = (InjectionInfo) iterator.next();
            injectInfo.inject();
        }

        iterator = this.injectors.iterator();
    ...
    public List generateAccessors() {
        Iterator methods = this.accessors.iterator();

        while (methods.hasNext()) {
            AccessorInfo accessor = (AccessorInfo) methods.next();

            accessor.locate();
        }

        ArrayList methods1 = new ArrayList();
        Iterator accessor2 = this.accessors.iterator();

        while (accessor2.hasNext()) {
            AccessorInfo accessor1 = (AccessorInfo) accessor2.next();
            MethodNode generated = accessor1.generate();

            this.getTarget().addMixinMethod(generated);
            methods1.add(generated);
        }
