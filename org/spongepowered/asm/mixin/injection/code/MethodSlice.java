...
public final class MethodSlice {
    ...
    public ReadOnlyInsnList getSlice(MethodNode method) {
        ...
        if (start > end) {
            throw new InvalidSliceException(this.owner, String.format("%s is negative size. Range(%d -> %d)", new Object[] { this.describe(), Integer.valueOf(start), Integer.valueOf(end)}));
    ...
    private int find(MethodNode method, InjectionPoint injectionPoint, int defaultValue, int failValue, String description) {
        if (injectionPoint == null) {
            ...
            if (nodes.size() != 1 && select == InjectionPoint.Selector.ONE) {
                throw new InvalidSliceException(this.owner, String.format("%s requires 1 result but found %d", new Object[] { this.describe(description), Integer.valueOf(nodes.size())}));
            } else if (!result) {
    ...
    static final class InsnListSlice extends ReadOnlyInsnList {
        ...
        public boolean contains(AbstractInsnNode insn) {
            AbstractInsnNode[] aabstractinsnnode = this.toArray();
            int i = aabstractinsnnode.length;

            for (int j = 0; j < i; ++j) {
                AbstractInsnNode node = aabstractinsnnode[j];
