    ...
    protected void inject(Target target, InjectionNodes.InjectionNode node) {
        if (node.isReplaced()) {
            ...
            if (!handlerDesc.equals(this.methodNode.desc)) {
                ...
                try {
                    int ex = this.discriminator.findLocal(context);

                    if (ex > -1) {
                        this.inject(context, ex);
                    }
