...
public class CallbackInjector extends Injector {
    ...
    protected void addTargetNode(Target target, List myNodes, AbstractInsnNode node, Set nominators) {
        ...
        Iterator iterator = nominators.iterator();

        while (iterator.hasNext()) {
            InjectionPoint ip = (InjectionPoint) iterator.next();
            String id = ip.getId();
        ...
        if (this.localCapture.isPrintLocals()) {
            ...
            if (!callback.checkDescriptor(this.methodNode.desc)) {
                ...
                if (callback.canCaptureLocals) {
                    MethodNode returnableSig = Bytecode.findMethod(this.classNode, this.methodNode.name, callback.getDescriptor());

                    if (returnableSig != null && Annotations.getVisible(returnableSig, Surrogate.class) != null) {
                        callbackMethod = returnableSig;
                    } else {
                        String surrogateHandler = this.generateBadLVTMessage(callback);

                        switch (this.localCapture) {
                        case CAPTURE_FAILEXCEPTION:
                            Injector.logger.error("Injection error: {}", new Object[] { surrogateHandler});
                            callbackMethod = this.generateErrorMethod(callback, "org/spongepowered/asm/mixin/injection/throwables/InjectionError", surrogateHandler);
                            break;

                        case CAPTURE_FAILSOFT:
                            Injector.logger.warn("Injection warning: {}", new Object[] { surrogateHandler});
                            return;

                        default:
                            Injector.logger.error("Critical injection failure: {}", new Object[] { surrogateHandler});
                            throw new InjectionError(surrogateHandler);
                        }
                    }
                } else {
                    String returnableSig1 = this.methodNode.desc.replace("Lorg/spongepowered/asm/mixin/injection/callback/CallbackInfo;", "Lorg/spongepowered/asm/mixin/injection/callback/CallbackInfoReturnable;");

                    if (callback.checkDescriptor(returnableSig1)) {
                        throw new InvalidInjectionException(this.info, "Invalid descriptor on " + this.info + "! CallbackInfoReturnable is required!");
                    }

                    MethodNode surrogateHandler1 = Bytecode.findMethod(this.classNode, this.methodNode.name, callback.getDescriptor());

                    if (surrogateHandler1 == null || Annotations.getVisible(surrogateHandler1, Surrogate.class) == null) {
                    ...
                    callbackMethod = surrogateHandler1;
                }
            }
    ...
    private void printLocals(CallbackInjector.Callback callback) {
        ...
        printer.kv("Instruction", "%s %s", new Object[] { callback.node.getClass().getSimpleName(), Bytecode.getOpcodeName(callback.node.getCurrentTarget().getOpcode())});
        printer.hr();
        if (callback.locals.length > callback.frameSize) {
            ...
            for (int l = 0; l < callback.locals.length; ++l) {
                ...
                if (callback.locals[l] != null) {
                    printer.add("%s [%3d]  %20s  %-50s %s", new Object[] { marker, Integer.valueOf(l), SignaturePrinter.getTypeName(callback.localTypes[l], false), meltSnowman(l, callback.locals[l].name), l >= callback.frameSize ? "<capture>" : ""});
                } else {
    ...
    static String meltSnowman(int index, String s) {
        return s != null && 9731 == s.charAt(0) ? "var" + index : s;
    }
