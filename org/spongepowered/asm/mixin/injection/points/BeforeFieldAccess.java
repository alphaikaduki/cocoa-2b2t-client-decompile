...
public class BeforeFieldAccess extends BeforeInvoke {
    ...
    protected boolean matchesInsn(AbstractInsnNode insn) {
        return insn instanceof FieldInsnNode && (((FieldInsnNode) insn).getOpcode() == this.opcode || this.opcode == -1) ? (this.arrOpcode == 0 ? true : (insn.getOpcode() != 178 && insn.getOpcode() != 180 ? false : Type.getType(((FieldInsnNode) insn).desc).getSort() == 9)) : false;
    }
    ...
    protected boolean addInsn(InsnList insns, Collection nodes, AbstractInsnNode insn) {
        if (this.arrOpcode > 0) {
            ...
            this.log("{} > > > > searching for array access opcode {} fuzz={}", new Object[] { this.className, Bytecode.getOpcodeName(accOpcode), Integer.valueOf(this.fuzzFactor)});
    ...
    public static AbstractInsnNode findArrayNode(InsnList insns, FieldInsnNode fieldNode, int opcode, int searchRange) {
        int pos = 0;
        ListIterator iter = insns.iterator(insns.indexOf(fieldNode) + 1);
