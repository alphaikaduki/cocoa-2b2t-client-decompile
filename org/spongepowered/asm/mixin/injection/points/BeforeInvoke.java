...
public class BeforeInvoke extends InjectionPoint {
    ...
    private String getClassName() {
        ...
        return String.format("@At(%s)", new Object[] { atCode != null ? atCode.value() : this.getClass().getSimpleName().toUpperCase()});
    }
    ...
    public boolean find(String desc, InsnList insns, Collection nodes) {
        this.log("{} is searching for an injection point in method with descriptor {}", new Object[] { this.className, desc});
        ...
        if (member == null) {
            ...
            for (ListIterator iter = insns.iterator(); iter.hasNext(); this.inspectInsn(desc, insns, insn)) {
                ...
                if (this.matchesInsn(insn)) {
                    ...
                    this.log("{} is considering insn {}", new Object[] { this.className, nodeInfo});
                    if (target.matches(nodeInfo.owner, nodeInfo.name, nodeInfo.desc)) {
                        this.log("{} > found a matching insn, checking preconditions...", new Object[] { this.className});
                        if (this.matchesInsn(nodeInfo, ordinal)) {
                            this.log("{} > > > found a matching insn at ordinal {}", new Object[] { this.className, Integer.valueOf(ordinal)});
    ...
    protected boolean matchesInsn(MemberInfo nodeInfo, int ordinal) {
        this.log("{} > > comparing target ordinal {} with current ordinal {}", new Object[] { this.className, Integer.valueOf(this.ordinal), Integer.valueOf(ordinal)});
