...
public class ModifyArgInjector extends InvokeInjector {
    ...
    protected void injectAtInvoke(Target target, InjectionNodes.InjectionNode node) {
        ...
        boolean extraLocals = false;
        int extraLocals1;

        if (this.singleArgMode) {
            extraLocals1 = this.injectSingleArgHandler(target, args, argIndex, insns);
        } else {
            extraLocals1 = this.injectMultiArgHandler(target, args, argIndex, insns);
        }

        target.insns.insertBefore(methodNode, insns);
        target.addToLocals(extraLocals1);
        target.addToStack(2 - (extraLocals1 - 1));
    }
