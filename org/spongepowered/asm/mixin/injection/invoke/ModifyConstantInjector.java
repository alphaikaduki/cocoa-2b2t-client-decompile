...
public class ModifyConstantInjector extends RedirectInjector {
    ...
    private void checkNarrowing(Target target, AbstractInsnNode constNode, Type constantType) {
        ...
        if (pop != null) {
            if (pop instanceof FieldInsnNode) {
                FieldInsnNode fieldinsnnode = (FieldInsnNode) pop;
                Type localVar = Type.getType(fieldinsnnode.desc);

                this.checkNarrowing(target, constNode, constantType, localVar, target.indexOf(pop), String.format("%s %s %s.%s", new Object[] { Bytecode.getOpcodeName(pop), SignaturePrinter.getTypeName(localVar, false), fieldinsnnode.owner.replace('/', '.'), fieldinsnnode.name}));
                ...
                int i = ((VarInsnNode) pop).var;
                LocalVariableNode localVar1 = Locals.getLocalVariableAt(target.classNode, target.method, pop, i);

                if (localVar1 != null && localVar1.desc != null) {
                    String name = localVar1.name != null ? localVar1.name : "unnamed";
                    Type localType = Type.getType(localVar1.desc);

                    this.checkNarrowing(target, constNode, constantType, localType, target.indexOf(pop), String.format("ISTORE[var=%d] %s %s", new Object[] { Integer.valueOf(i), SignaturePrinter.getTypeName(localType, false), name}));
                }
            }
    ...
    private void checkNarrowing(Target target, AbstractInsnNode constNode, Type constantType, Type type, int index, String description) {
        ...
        if (toSort < fromSort) {
            ...
            Injector.logger.log(level, "Narrowing conversion of <{}> to <{}> in {} target {} at opcode {} ({}){}", new Object[] { fromType, toType, this.info, target, Integer.valueOf(index), description, message});
        }
