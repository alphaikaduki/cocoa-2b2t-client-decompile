...
public abstract class InjectionPoint {
    ...
    public abstract boolean find(String s, InsnList insnlist, Collection collection);

    public String toString() {
        return String.format("@At(\"%s\")", new Object[] { this.getAtCode()});
    }
    ...
    public static InjectionPoint parse(IMixinContext context, MethodNode method, AnnotationNode parent, AnnotationNode node) {
        ...
        Object args = (List) Annotations.getValue(node, "args");
        ...
        int by = ((Integer) Annotations.getValue(node, "by", (Object) Integer.valueOf(0))).intValue();
        int ordinal = ((Integer) Annotations.getValue(node, "ordinal", (Object) Integer.valueOf(-1))).intValue();
        int opcode = ((Integer) Annotations.getValue(node, "opcode", (Object) Integer.valueOf(0))).intValue();
        ...
        return parse(context, method, parent, at, shift, by, (List) args, target, slice, ordinal, opcode, id);
    }
    ...
    private static Class findClass(IMixinContext context, InjectionPointData data) {
        ...
        Class ipClass = (Class) InjectionPoint.types.get(type);
        ...
        if (ipClass == null) {
            ...
            try {
                ...
                throw new InvalidInjectionException(context, data + " could not be loaded or is not a valid InjectionPoint", exception);
            }
        }
    ...
    private static InjectionPoint create(IMixinContext context, InjectionPointData data, Class ipClass) {
        ...
        try {
            ...
            throw new InvalidInjectionException(context, ipClass.getName() + " must contain a constructor which accepts an InjectionPointData", nosuchmethodexception);
        }

        InjectionPoint point = null;

        try {
            point = (InjectionPoint) ipCtor.newInstance(new Object[] { data});
            return point;
        } catch (Exception exception) {
            throw new InvalidInjectionException(context, "Error whilst instancing injection point " + ipClass.getName() + " for " + data.getAt(), exception);
        }
    }
    ...
    private static void validateByValue(IMixinContext context, MethodNode method, AnnotationNode parent, InjectionPoint point, int by) {
        ...
        if (err != InjectionPoint.ShiftByViolationBehaviour.IGNORE) {
            ...
            if (by > allowed) {
                ...
                String message = String.format("@%s(%s) Shift.BY=%d on %s::%s exceeds %s%d. %s", new Object[] { Bytecode.getSimpleName(parent), point, Integer.valueOf(by), context, method.name, limitBreached, Integer.valueOf(allowed), advice});
    ...
    public static void register(Class type) {
        ...
        if (code == null) {
            ...
            Class existing = (Class) InjectionPoint.types.get(code.value());
    ...
    static final class Shift extends InjectionPoint {
        ...
        public boolean find(String desc, InsnList insns, Collection nodes) {
            Object list = nodes instanceof List ? (List) nodes : new ArrayList(nodes);
    ...
    static final class Intersection extends InjectionPoint.CompositeInjectionPoint {
        ...
        public boolean find(String desc, InsnList insns, Collection nodes) {
            ...
            for (int alpha = 0; alpha < this.components.length; ++alpha) {
                allNodes[alpha] = new ArrayList();
                this.components[alpha].find(desc, insns, allNodes[alpha]);
            }

            ArrayList arraylist = allNodes[0];

            for (int nodeIndex = 0; nodeIndex < arraylist.size(); ++nodeIndex) {
                AbstractInsnNode node = (AbstractInsnNode) arraylist.get(nodeIndex);
                boolean in = true;
    ...
    public static enum Selector {
        ...
        public static final InjectionPoint.Selector DEFAULT = InjectionPoint.Selector.FIRST;
    }
