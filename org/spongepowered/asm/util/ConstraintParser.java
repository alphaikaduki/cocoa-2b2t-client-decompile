...
public final class ConstraintParser {
    ...
    public static ConstraintParser.Constraint parse(String expr) {
        if (expr != null && expr.length() != 0) {
            ...
            String[] astring = exprs;
            int i = exprs.length;

            for (int j = 0; j < i; ++j) {
                String subExpr = astring[j];
    ...
    public static class Constraint {
        ...
        Constraint(String expr) {
            this.expr = expr;
            Matcher matcher = ConstraintParser.Constraint.pattern.matcher(expr);
            ...
            if (this.has(1)) {
                ...
                if (this.has(4)) {
                    ...
                    if (hasModifier) {
                        ...
                        trailing = this.elem(0);
                        if (">".equals(trailing)) {
                            ++this.min;
                            this.max = Integer.MAX_VALUE;
                        } else if (">=".equals(trailing)) {
                            this.max = Integer.MAX_VALUE;
                        } else if ("<".equals(trailing)) {
                            this.max = --this.min;
        ...
        public void check(ITokenProvider environment) throws ConstraintViolationException {
            if (this != ConstraintParser.Constraint.NONE) {
