...
public final class Bytecode {
    ...
    public static AbstractInsnNode findInsn(MethodNode method, int opcode) {
        ListIterator findReturnIter = method.instructions.iterator();

        AbstractInsnNode insn;

        do {
            if (!findReturnIter.hasNext()) {
                return null;
            }

            insn = (AbstractInsnNode) findReturnIter.next();
        } while (insn.getOpcode() != opcode);
    ...
    public static String describeNode(AbstractInsnNode node) {
        if (node == null) {
            ...
            if (node instanceof JumpInsnNode) {
                ...
                MethodInsnNode ln = (MethodInsnNode) node;

                out = out + String.format("[%s] %s %s %s", new Object[] { getOpcodeName(node), ln.owner, ln.name, ln.desc});
            } else if (node instanceof FieldInsnNode) {
                FieldInsnNode ln1 = (FieldInsnNode) node;

                out = out + String.format("[%s] %s %s %s", new Object[] { getOpcodeName(node), ln1.owner, ln1.name, ln1.desc});
            } else if (node instanceof LineNumberNode) {
                LineNumberNode ln2 = (LineNumberNode) node;

                out = out + String.format("LINE=[%d] LABEL=[%s]", new Object[] { Integer.valueOf(ln2.line), ln2.start.getLabel()});
    ...
    private static String getOpcodeName(int opcode, String start, int min) {
        if (opcode >= min) {
            ...
            try {
                Field[] afield = Opcodes.class.getDeclaredFields();
                int i = afield.length;
    ...
    public static Map cloneLabels(InsnList source) {
        ...
        ListIterator iter = source.iterator();
    ...
    private static String toDescriptor(Object arg) {
        return arg instanceof String ? (String) arg : (arg instanceof Type ? arg.toString() : (arg instanceof Class ? Type.getDescriptor((Class) arg) : (arg == null ? "" : arg.toString())));
    }
    ...
    public static String changeDescriptorReturnType(String desc, String returnType) {
        return desc == null ? null : (returnType == null ? desc : desc.substring(0, desc.lastIndexOf(41) + 1) + returnType);
    }
    ...
    public static boolean isConstant(AbstractInsnNode insn) {
        return insn == null ? false : Ints.contains(Bytecode.CONSTANTS_ALL, insn.getOpcode());
    }
    ...
    public static Object getConstant(AbstractInsnNode insn) {
        if (insn == null) {
            ...
            if (insn instanceof IntInsnNode) {
                index = ((IntInsnNode) insn).operand;
                ...
                index = Ints.indexOf(Bytecode.CONSTANTS_ALL, insn.getOpcode());
    ...
    public static Type getConstantType(AbstractInsnNode insn) {
        if (insn == null) {
            ...
            Object index1 = ((LdcInsnNode) insn).cst;

            if (index1 instanceof Integer) {
                return Type.getType("I");
            } else if (index1 instanceof Float) {
                return Type.getType("F");
            } else if (index1 instanceof Long) {
                return Type.getType("J");
            } else if (index1 instanceof Double) {
                return Type.getType("D");
            } else if (index1 instanceof String) {
                return Type.getType("Ljava/lang/String;");
            } else if (index1 instanceof Type) {
                return Type.getType("Ljava/lang/Class;");
            } else {
                throw new IllegalArgumentException("LdcInsnNode with invalid payload type " + index1.getClass() + " in getConstant");
            }
        } else {
    ...
    private static Bytecode.Visibility getVisibility(int flags) {
        return (flags & 4) != 0 ? Bytecode.Visibility.PROTECTED : ((flags & 2) != 0 ? Bytecode.Visibility.PRIVATE : ((flags & 1) != 0 ? Bytecode.Visibility.PUBLIC : Bytecode.Visibility.PACKAGE));
    }
    ...
    private static List mergeAnnotations(List from, List to, String type, String name) {
        try {
            ...
            Iterator ex = from.iterator();

            while (ex.hasNext()) {
                AnnotationNode annotation = (AnnotationNode) ex.next();

                if (isMergeableAnnotation(annotation)) {
                    Iterator iter = ((List) to).iterator();
    ...
    private static boolean isMergeableAnnotation(AnnotationNode annotation) {
        return annotation.desc.startsWith("L" + Constants.MIXIN_PACKAGE_REF) ? Bytecode.mergeableAnnotationPattern.matcher(annotation.desc).matches() : true;
    }
        ...
        for (index = 0; ia.hasNext() && ib.hasNext(); ++index) {
            ...
            if (!(na instanceof LabelNode)) {
                if (na instanceof MethodInsnNode) {
                    MethodInsnNode ta = (MethodInsnNode) na;
                    MethodInsnNode tb = (MethodInsnNode) nb;
                    ...
                    if (na instanceof VarInsnNode) {
                        VarInsnNode varinsnnode = (VarInsnNode) na;
                        VarInsnNode varinsnnode1 = (VarInsnNode) nb;

                        if (varinsnnode2.var != varinsnnode3.var) {
                        ...
                        TypeInsnNode typeinsnnode = (TypeInsnNode) na;
                        TypeInsnNode typeinsnnode1 = (TypeInsnNode) nb;

                        if (typeinsnnode.getOpcode() == 192 && !typeinsnnode.desc.equals(typeinsnnode1.desc)) {
    ...
    public static enum Visibility {
        ...
        PRIVATE(2), PROTECTED(4), PACKAGE(0), PUBLIC(1);
