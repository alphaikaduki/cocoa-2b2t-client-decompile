...
public class PrettyPrinter {
    ...
    public PrettyPrinter table(String... titles) {
        this.table = new PrettyPrinter.Table();
        String[] astring = titles;
        int i = titles.length;

        for (int j = 0; j < i; ++j) {
            String title = astring[j];
    ...
    public PrettyPrinter table(Object... format) {
        ...
        Object[] aobject = format;
        int i = format.length;

        for (int j = 0; j < i; ++j) {
            Object entry = aobject[j];
    ...
    public PrettyPrinter add(Object[] array, String format) {
        Object[] aobject = array;
        int i = array.length;

        for (int j = 0; j < i; ++j) {
            Object element = aobject[j];

            this.add(format, new Object[] { element});
        }
    ...
    public PrettyPrinter addIndexed(Object[] array) {
        ...
        for (int index = 0; index < array.length; ++index) {
            this.add(format, new Object[] { Integer.valueOf(index), array[index]});
        }
    ...
    public PrettyPrinter add(Throwable th, int indent) {
        while (th != null) {
            this.add("%s: %s", new Object[] { th.getClass().getName(), th.getMessage()});
    ...
    public PrettyPrinter add(StackTraceElement[] stackTrace, int indent) {
        ...
        for (int j = 0; j < i; ++j) {
            StackTraceElement st = astacktraceelement[j];

            this.add("%s%s", new Object[] { margin, st});
        }
    ...
    private PrettyPrinter append(Object object, int indent, String margin) {
        if (object instanceof String) {
            return this.add("%s%s", new Object[] { margin, object});
        } else if (!(object instanceof Iterable)) {
            if (object instanceof Map) {
                this.kvWidth(indent);
                return this.add((Map) object);
            } else {
                return object instanceof PrettyPrinter.IPrettyPrintable ? this.add((PrettyPrinter.IPrettyPrintable) object) : (object instanceof Throwable ? this.add((Throwable) object, indent) : (object.getClass().isArray() ? this.add((Object[]) ((Object[]) object), indent + "%s") : this.add("%s%s", new Object[] { margin, object})));
            }
        } else {
    ...
    public PrettyPrinter addWrapped(int width, String format, Object... args) {
        ...
        try {
            Iterator ex = this.getWrapped(width, line, indent).iterator();

            while (ex.hasNext()) {
                String wrappedLine = (String) ex.next();
    ...
    static class Table implements PrettyPrinter.IVariableWidthEntry {
        ...
        void updateFormat() {
            ...
            while (iterator.hasNext()) {
                PrettyPrinter.Column column = (PrettyPrinter.Column) iterator.next();
