...
public class SyntheticBridgeException extends MixinException {
    ...
    private PrettyPrinter printProblem(PrettyPrinter printer, IMixinContext context, MethodNode mda, MethodNode mdb) {
        ...
        case BAD_LOAD:
            ...
            for (int i = 0; ia.hasNext() && ib.hasNext(); ++i) {
                AbstractInsnNode abstractinsnnode = (AbstractInsnNode) ia.next();
                AbstractInsnNode abstractinsnnode1 = (AbstractInsnNode) ib.next();
                ...
                if (abstractinsnnode instanceof VarInsnNode && abstractinsnnode1 instanceof VarInsnNode) {
                    ...
                    Type type1 = varinsnnode2.var > 0 ? argsa[varinsnnode3.var - 1] : target;

                    type = varinsnnode4.var > 0 ? argsb[varinsnnode5.var - 1] : target;
                    printer.kv("Target " + i, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName(varinsnnode6), Integer.valueOf(varinsnnode7.var), type1});
                    printer.kv("Incoming " + i, "%8s %-2d %s", new Object[] { Bytecode.getOpcodeName(varinsnnode8), Integer.valueOf(varinsnnode9.var), type});
                    if (type1.equals(type)) {
                        printer.kv("", "Types match: %s", new Object[] { type1});
                    } else if (type1.getSort() != type.getSort()) {
                        printer.kv("", "Types are incompatible");
                    } else if (type1.getSort() == 10) {
                        ClassInfo classinfo = ClassInfo.getCommonSuperClassOrInterface(type1, type);

                        printer.kv("", "Common supertype: %s", new Object[] { classinfo});
                    }
        ...
        case BAD_INVOKE_DESC:
            ...
            if (arga.length != argb.length) {
                int rta = Type.getArgumentTypes(mda.desc).length;
                String rtb = arga.length == rta ? "The TARGET" : (argb.length == rta ? " The INCOMING" : "NEITHER");
                ...
                printer.add("unrecoverable error. %s method has the expected arg count of %d", new Object[] { rtb, Integer.valueOf(rta)});
                break;
            } else {
                type = Type.getReturnType(mdna.desc);
                Type type2 = Type.getReturnType(mdnb.desc);
                ...
                this.printTypeComparison(printer, "return type", type, type2);
    ...
    public static enum Problem {
        ...
        BAD_INSN("Conflicting opcodes %4$s and %5$s at offset %3$d in synthetic bridge method %1$s%2$s"), BAD_LOAD("Conflicting variable access at offset %3$d in synthetic bridge method %1$s%2$s"), BAD_CAST("Conflicting type cast at offset %3$d in synthetic bridge method %1$s%2$s"), BAD_INVOKE_NAME("Conflicting synthetic bridge target method name in synthetic bridge method %1$s%2$s Existing:%6$s Incoming:%7$s"), BAD_INVOKE_DESC("Conflicting synthetic bridge target method descriptor in synthetic bridge method %1$s%2$s Existing:%8$s Incoming:%9$s"), BAD_LENGTH("Mismatched bridge method length for synthetic bridge method %1$s%2$s unexpected extra opcode at offset %3$d");
        ...
        String getMessage(String name, String desc, int index, AbstractInsnNode a, AbstractInsnNode b) {
            return String.format(this.message, new Object[] { name, desc, Integer.valueOf(index), Bytecode.getOpcodeName(a), Bytecode.getOpcodeName(a), getInsnName(a), getInsnName(b), getInsnDesc(a), getInsnDesc(b)});
        }
