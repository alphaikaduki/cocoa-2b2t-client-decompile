...
public class ReferenceManager implements IReferenceManager {
    ...
    private PrintWriter newWriter(String fileName, String description) throws IOException {
        if (fileName.matches("^.*[\\\\/:].*$")) {
            File outResource1 = new File(fileName);

            outResource1.getParentFile().mkdirs();
            this.ap.printMessage(Kind.NOTE, "Writing " + description + " to " + outResource1.getAbsolutePath());
            return new PrintWriter(outResource1);
        } else {
