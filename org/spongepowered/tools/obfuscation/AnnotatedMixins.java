...
final class AnnotatedMixins implements IMixinAnnotationProcessor, ITokenProvider, ITypeHandleProvider, IJavadocProvider {
    ...
    private void initTokenCache(String tokens) {
        if (tokens != null) {
            ...
            String[] astring = tokenValues;
            int i = tokenValues.length;

            for (int j = 0; j < i; ++j) {
                String tokenValue = astring[j];
    ...
    public Collection getMixinsTargeting(TypeElement targetType) {
        ...
        Iterator iterator = this.targets.getMixinsTargeting(targetType).iterator();

        while (iterator.hasNext()) {
            TypeReference mixin = (TypeReference) iterator.next();
    ...
    public void onPassCompleted(RoundEnvironment roundEnv) {
        ...
        Iterator iterator = ((Collection) (roundEnv.processingOver() ? this.mixins.values() : this.mixinsForPass)).iterator();

        while (iterator.hasNext()) {
            AnnotatedMixin mixin = (AnnotatedMixin) iterator.next();
    ...
    public static AnnotatedMixins getMixinsForEnvironment(ProcessingEnvironment processingEnv) {
        AnnotatedMixins mixins = (AnnotatedMixins) AnnotatedMixins.instances.get(processingEnv);
