...
public class ObfuscationDataProvider implements IObfuscationDataProvider {
    ...
    public ObfuscationData getObfEntryRecursive(MemberInfo targetMember) {
        ...
        try {
            while (obfData.isEmpty()) {
                TypeHandle ex = this.ap.getTypeProvider().getTypeHandle(currentTarget.owner);

                if (ex == null) {
                    return obfData;
                }

                TypeHandle superClass = ex.getSuperclass();
                ...
                Iterator iterator = ex.getInterfaces().iterator();

                while (iterator.hasNext()) {
                    TypeHandle iface = (TypeHandle) iterator.next();
    ...
    private ObfuscationData getRemappedMethod(MemberInfo method, boolean remapDescriptor) {
        ...
        Iterator iterator = this.environments.iterator();

        while (iterator.hasNext()) {
            ObfuscationEnvironment env = (ObfuscationEnvironment) iterator.next();
