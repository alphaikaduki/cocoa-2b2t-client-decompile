...
abstract class AnnotatedMixinElementHandler {
    ...
    protected final void addFieldMappings(String mcpName, String mcpSignature, ObfuscationData obfData) {
        Iterator iterator = obfData.iterator();
    ...
    protected final void checkConstraints(ExecutableElement method, AnnotationHandle annotation) {
        try {
            ConstraintParser.Constraint ex = ConstraintParser.parse((String) annotation.getValue("constraints"));

            try {
                ex.check(this.ap.getTokenProvider());
            } catch (ConstraintViolationException constraintviolationexception) {
                this.ap.printMessage(Kind.ERROR, constraintviolationexception.getMessage(), method, annotation.asMirror());
            }
        } catch (InvalidConstraintException invalidconstraintexception) {
            this.ap.printMessage(Kind.WARNING, invalidconstraintexception.getMessage(), method, annotation.asMirror());
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (!target.isImaginary()) {
                ...
                if (targetMethod == null && name.hasAliases()) {
                    Iterator iterator1 = name.getAliases().iterator();
