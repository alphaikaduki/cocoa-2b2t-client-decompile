...
public class AnnotatedMixinElementHandlerAccessor extends AnnotatedMixinElementHandler implements IMixinContext {
    ...
    private void registerAccessorForTarget(AnnotatedMixinElementHandlerAccessor.AnnotatedElementAccessor elem, TypeHandle target) {
        ...
        if (elem.shouldRemap()) {
            ...
            if (obfData.isEmpty()) {
                String ex = this.mixin.isMultiTarget() ? " in target " + target : "";

                elem.printMessage(this.ap, Kind.WARNING, "Unable to locate obfuscation mapping" + ex + " for @Accessor target " + elem);
            } else {
                ...
                try {
                    ...
                    elem.printMessage(this.ap, Kind.ERROR, "Mapping conflict for @Accessor target " + elem + ": " + referencemanager_referenceconflictexception.getNew() + " for target " + target + " conflicts with existing mapping " + referencemanager_referenceconflictexception.getOld());
                }
        ...
        if (elem.shouldRemap()) {
            ...
            if (obfData.isEmpty()) {
                ...
                try {
                    ...
                    elem.printMessage(this.ap, Kind.ERROR, "Mapping conflict for @Invoker target " + elem + ": " + referencemanager_referenceconflictexception.getNew() + " for target " + target + " conflicts with existing mapping " + referencemanager_referenceconflictexception.getOld());
                }
