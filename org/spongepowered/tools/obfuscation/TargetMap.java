...
public final class TargetMap extends HashMap {
    ...
    private Set getMixinsFor(TypeReference target) {
        Object mixins = (Set) this.get(target);
    ...
    public void readImports(File file) throws IOException {
        if (file.isFile()) {
            Iterator iterator = Files.readLines(file, Charset.defaultCharset()).iterator();
    ...
    public void write(boolean temp) {
        ...
        try {
            File ex = getSessionFile(this.sessionId);

            if (temp) {
                ex.deleteOnExit();
            }

            fout = new FileOutputStream(ex, true);
