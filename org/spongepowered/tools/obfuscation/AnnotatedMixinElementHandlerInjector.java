...
class AnnotatedMixinElementHandlerInjector extends AnnotatedMixinElementHandler {
        ...
        while (iterator.hasNext()) {
            ...
            if (targetMember.name != null) {
                try {
                    ...
                    elem.printMessage(this.ap, Kind.ERROR, invalidmemberdescriptorexception.getMessage());
                }
                ...
                if (elem.shouldRemap()) {
                    Iterator ex = this.mixin.getTargets().iterator();

                    while (ex.hasNext()) {
                        TypeHandle target = (TypeHandle) ex.next();
    ...
    private boolean registerInjector(AnnotatedMixinElementHandlerInjector.AnnotatedElementInjector elem, String reference, MemberInfo targetMember, TypeHandle target) {
        ...
        if (desc == null) {
            Kind targetName1 = this.mixin.isMultiTarget() ? Kind.ERROR : Kind.WARNING;
            ...
            if (target.isSimulated()) {
                ...
                elem.printMessage(this.ap, targetName1, elem + " target requires method signature because enclosing type information for " + target + " is unavailable");
            } else if (!targetMember.isInitialiser()) {
                elem.printMessage(this.ap, targetName1, "Unable to determine signature for " + elem + " target method");
            }
            ...
            if (obfData.isEmpty()) {
                if (!target.isSimulated()) {
                    ...
                    Kind refMap1 = targetMember.isConstructor() ? Kind.WARNING : Kind.ERROR;

                    elem.addMessage(refMap1, "No obfuscation mapping for " + targetName, elem.getElement(), elem.getAnnotation());
                    return false;
                }
            ...
            try {
            ...
            } catch (ReferenceManager.ReferenceConflictException referencemanager_referenceconflictexception) {
                ...
                if (elem.hasCoerceArgument() && targetMember.owner == null && targetMember.desc == null) {
                    MemberInfo oldMember = MemberInfo.parse(referencemanager_referenceconflictexception.getOld());
                    MemberInfo newMember = MemberInfo.parse(referencemanager_referenceconflictexception.getNew());
                ...
                elem.printMessage(this.ap, Kind.ERROR, conflictType + " reference conflict for " + targetName + ": " + reference + " -> " + referencemanager_referenceconflictexception.getNew() + " previously defined as " + referencemanager_referenceconflictexception.getOld());
            }
    ...
    public void registerInjectionPoint(AnnotatedMixinElementHandlerInjector.AnnotatedElementInjectionPoint elem, String format) {
        ...
        if (elem.shouldRemap()) {
            ...
            if ("NEW".equals(type)) {
                this.remapNewTarget(String.format(format, new Object[] { type + ".<target>"}), target, elem);
                this.remapNewTarget(String.format(format, new Object[] { type + ".args[class]"}), elem.getAtArg("class"), elem);
            } else {
                this.remapReference(String.format(format, new Object[] { type + ".<target>"}), target, elem);
            }
    ...
    protected final void remapNewTarget(String subject, String reference, AnnotatedMixinElementHandlerInjector.AnnotatedElementInjectionPoint elem) {
        if (reference != null) {
            ...
            if (target != null) {
                ...
                Iterator iterator = remapped.iterator();
    ...
    protected final void remapReference(String subject, String reference, AnnotatedMixinElementHandlerInjector.AnnotatedElementInjectionPoint elem) {
        if (reference != null) {
            ...
            if (!targetMember.isFullyQualified()) {
                String ex1 = targetMember.owner == null ? (targetMember.desc == null ? "owner and signature" : "owner") : "signature";

                this.ap.printMessage(Kind.ERROR, subject + " is not fully qualified, missing " + ex1, elem.getElement(), errorsOn);
            } else {
                try {
                    targetMember.validate();
                } catch (InvalidMemberDescriptorException invalidmemberdescriptorexception) {
                    this.ap.printMessage(Kind.ERROR, invalidmemberdescriptorexception.getMessage(), elem.getElement(), errorsOn);
                }
                ...
                try {
                    ...
                    if (targetMember.isField()) {
                        ex = this.obf.getDataProvider().getObfFieldRecursive(targetMember);
                        if (ex.isEmpty()) {
                            this.ap.printMessage(Kind.WARNING, "Cannot find field mapping for " + subject + " \'" + reference + "\'", elem.getElement(), errorsOn);
                            return;
                        }

                        this.obf.getReferenceManager().addFieldMapping(this.classRef, reference, targetMember, ex);
                    } else {
                        ex = this.obf.getDataProvider().getObfMethodRecursive(targetMember);
                        if (ex.isEmpty() && (targetMember.owner == null || !targetMember.owner.startsWith("java/lang/"))) {
                            this.ap.printMessage(Kind.WARNING, "Cannot find method mapping for " + subject + " \'" + reference + "\'", elem.getElement(), errorsOn);
                            return;
                        }

                        this.obf.getReferenceManager().addMethodMapping(this.classRef, reference, targetMember, ex);
                    }
                } catch (ReferenceManager.ReferenceConflictException referencemanager_referenceconflictexception) {
                    this.ap.printMessage(Kind.ERROR, "Unexpected reference conflict for " + subject + ": " + reference + " -> " + referencemanager_referenceconflictexception.getNew() + " previously defined as " + referencemanager_referenceconflictexception.getOld(), elem.getElement(), errorsOn);
                    return;
                }
    ...
    static class AnnotatedElementInjector extends AnnotatedMixinElementHandler.AnnotatedElement {
        ...
        public boolean hasCoerceArgument() {
            if (!this.annotation.toString().equals("@Inject")) {
                ...
                Iterator iterator = ((ExecutableElement) this.element).getParameters().iterator();

                if (iterator.hasNext()) {
                    VariableElement name = (VariableElement) iterator.next();
