...
public abstract class ObfuscationEnvironment implements IObfuscationEnvironment {
    ...
    public MappingMethod getObfMethod(MappingMethod method, boolean lazyRemap) {
        if (!this.initMappings()) {
            ...
            for (MappingMethod remappedOwner = method; remappedOwner != null && mapping == null; remappedOwner = remappedOwner.getSuper()) {
                mapping = this.mappingProvider.getMethodMapping(remappedOwner);
            }
            ...
            String remappedOwner1 = this.getObfClass(mapping.getOwner());

            if (remappedOwner1 != null && !remappedOwner1.equals(method.getOwner()) && !remappedOwner1.equals(mapping.getOwner())) {
                if (remapped) {
                    return mapping.move(remappedOwner1);
                } else {
                    String desc = ObfuscationUtil.mapDescriptor(mapping.getDesc(), this.remapper);

                    return new MappingMethod(remappedOwner1, mapping.getSimpleName(), desc);
                }
            } else {
    ...
    public MappingField getObfField(MappingField field, boolean lazyRemap) {
        if (!this.initMappings()) {
            ...
            return remappedOwner != null && !remappedOwner.equals(field.getOwner()) && !remappedOwner.equals(mapping.getOwner()) ? mapping.move(remappedOwner) : (mapping != field ? mapping : null);
        }
    }
    ...
    public void writeMappings(Collection consumers) {
        ...
        while (iterator.hasNext()) {
            IMappingConsumer mappings = (IMappingConsumer) iterator.next();
