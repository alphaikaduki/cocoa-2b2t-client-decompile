    ...
    private static MethodHandle findMethodRecursive(TypeHandle target, String name, String signature, String rawSignature, boolean matchCase) {
        ...
        if (elem == null) {
            ...
            if (method != null) {
                ...
                Iterator superClass = elem.getInterfaces().iterator();

                do {
                    if (!superClass.hasNext()) {
                        TypeMirror superClass1 = elem.getSuperclass();

                        if (superClass1 != null && superClass1.getKind() != TypeKind.NONE) {
                            return findMethodRecursive(superClass1, name, signature, rawSignature, matchCase);
                        }

                        return null;
                    }

                    TypeMirror iface = (TypeMirror) superClass.next();
