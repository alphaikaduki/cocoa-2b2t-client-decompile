...
public class EntityUtil3 implements Util {
    ...
    public static void attackEntity(Entity entity, boolean packet, boolean swingArm) {
        if (packet) {
            EntityUtil3.mc.player.connection.sendPacket(new CPacketUseEntity(entity));
        } else {
            EntityUtil3.mc.playerController.attackEntity(EntityUtil3.mc.player, entity);
        }
    ...
    public static boolean stopSneaking(boolean isSneaking) {
        if (isSneaking && EntityUtil3.mc.player != null) {
            EntityUtil3.mc.player.connection.sendPacket(new CPacketEntityAction(EntityUtil3.mc.player, Action.STOP_SNEAKING));
        }
    ...
    public static List getUnsafeBlocks(Entity entity, int height, boolean floor, boolean face) {
    ...
    public static boolean isMobAggressive(Entity entity) {
        if (entity instanceof EntityPigZombie) {
            ...
            if (entity instanceof EntityWolf) {
                return ((EntityWolf) entity).isAngry() && !EntityUtil3.mc.player.equals(((EntityWolf) entity).getOwner());
            }
    ...
    public static List getUnsafeBlocksFromVec3d(Vec3d pos, int height, boolean floor, boolean face) {
        ArrayList vec3ds = new ArrayList();
        Vec3d[] avec3d = getOffsets(height, floor, face);
        int i = avec3d.length;

        for (int j = 0; j < i; ++j) {
            Vec3d vector = avec3d[j];
            BlockPos targetPos = (new BlockPos(pos)).add(vector.x, vector.y, vector.z);
            Block block = EntityUtil3.mc.world.getBlockState(targetPos).getBlock();
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        Iterator iterator = EntityUtil3.mc.world.playerEntities.iterator();

        while (iterator.hasNext()) {
            EntityPlayer player = (EntityPlayer) iterator.next();

            if (EntityUtil3.mc.player.getDistanceSq(player) <= range * range && !OyVey.friendManager.isFriend(player)) {
                Vec3d[] avec3d = EntityUtil3.doubleLegOffsetList;
                int i = avec3d.length;
    ...
    public static boolean isObbyHole(BlockPos blockPos) {
        ...
        BlockPos[] ablockpos = array;
        int i = array.length;

        for (int j = 0; j < i; ++j) {
            BlockPos pos = ablockpos[j];
            IBlockState touchingState = EntityUtil3.mc.world.getBlockState(pos);
    ...
    public static Vec3d[] getUnsafeBlockArray(Entity entity, int height, boolean floor, boolean face) {
        List list = getUnsafeBlocks(entity, height, floor, face);
    ...
    public static Vec3d[] getUnsafeBlockArrayFromVec3d(Vec3d pos, int height, boolean floor, boolean face) {
        List list = getUnsafeBlocksFromVec3d(pos, height, floor, face);
        Vec3d[] array = new Vec3d[list.size()];
    ...
    public static List getUntrappedBlocks(EntityPlayer player, boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean face) {
        ArrayList vec3ds = new ArrayList();
    ...
    public static boolean isDrivenByPlayer(Entity entityIn) {
        return EntityUtil3.mc.player != null && entityIn != null && entityIn.equals(EntityUtil3.mc.player.getRidingEntity());
    }
    ...
    public static List getUntrappedBlocksExtended(int extension, EntityPlayer player, boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean raytrace, boolean noScaffoldExtend, boolean face) {
    ...
    public static List targets(Vec3d vec3d, boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean raytrace, boolean face) {
        ArrayList placeTargets = new ArrayList();
        ...
        if (antiStep) {
            ...
            List offsets = getUnsafeBlocksFromVec3d(vec3d, 2, false, face);

            if (offsets.size() == 4) {
                Iterator array = offsets.iterator();

                label38:
                while (array.hasNext()) {
                    Vec3d vector = (Vec3d) array.next();
        ...
        if (!face) {
            ArrayList offsets1 = new ArrayList();

            offsets1.add(new Vec3d(1.0D, 1.0D, 0.0D));
            offsets1.add(new Vec3d(0.0D, 1.0D, -1.0D));
            offsets1.add(new Vec3d(0.0D, 1.0D, 1.0D));
            Vec3d[] array1 = new Vec3d[offsets1.size()];

            placeTargets.removeAll(Arrays.asList(BlockUtil.convertVec3ds(vec3d, (Vec3d[]) offsets1.toArray(array1))));
        }

        return placeTargets;
    }

    public static List getOffsetList(int y, boolean floor, boolean face) {
    ...
    public static Vec3d[] getOffsets(int y, boolean floor, boolean face) {
        List offsets = getOffsetList(y, floor, face);
    ...
    public static Vec3d[] getTrapOffsets(boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean face) {
        List offsets = getTrapOffsetsList(antiScaffold, antiStep, legs, platform, antiDrop, face);
        Vec3d[] array = new Vec3d[offsets.size()];

        return (Vec3d[]) offsets.toArray(array);
    }

    public static List getTrapOffsetsList(boolean antiScaffold, boolean antiStep, boolean legs, boolean platform, boolean antiDrop, boolean face) {
        ArrayList offsets = new ArrayList(getOffsetList(1, false, face));
    ...
    public static boolean canEntityFeetBeSeen(Entity entityIn) {
        return EntityUtil3.mc.world.rayTraceBlocks(new Vec3d(EntityUtil3.mc.player.posX, EntityUtil3.mc.player.posX + (double) EntityUtil3.mc.player.getEyeHeight(), EntityUtil3.mc.player.posZ), new Vec3d(entityIn.posX, entityIn.posY, entityIn.posZ), false, true, false) == null;
    }

    public static boolean isntValid(Entity entity, double range) {
        return entity == null || isDead(entity) || entity.equals(EntityUtil3.mc.player) || entity instanceof EntityPlayer && OyVey.friendManager.isFriend(entity.getName()) || EntityUtil3.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static double getMaxSpeed() {
        ...
        if (EntityUtil3.mc.player.isPotionActive((Potion) Objects.requireNonNull(Potion.getPotionById(1)))) {
            maxModifier *= 1.0D + 0.2D * (double) (((PotionEffect) Objects.requireNonNull(EntityUtil3.mc.player.getActivePotionEffect((Potion) Objects.requireNonNull(Potion.getPotionById(1))))).getAmplifier() + 1);
        }
    ...
    public static boolean isEntityMoving(Entity entity) {
        return entity == null ? false : (entity instanceof EntityPlayer ? EntityUtil3.mc.gameSettings.keyBindForward.isKeyDown() || EntityUtil3.mc.gameSettings.keyBindBack.isKeyDown() || EntityUtil3.mc.gameSettings.keyBindLeft.isKeyDown() || EntityUtil3.mc.gameSettings.keyBindRight.isKeyDown() : entity.motionX != 0.0D || entity.motionY != 0.0D || entity.motionZ != 0.0D);
    }
    ...
    public static void moveEntityStrafe(double speed, Entity entity) {
        if (entity != null) {
            MovementInput movementInput = EntityUtil3.mc.player.movementInput;
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && OyVey.friendManager.isFriend((EntityPlayer) entity)) {
                new Color(0.33333334F, 1.0F, 1.0F, (float) alpha / 255.0F);
            }

            Killaura killaura = (Killaura) OyVey.moduleManager.getModuleByClass(Killaura.class);
    ...
    public static boolean isMoving() {
        return (double) EntityUtil3.mc.player.moveForward != 0.0D || (double) EntityUtil3.mc.player.moveStrafing != 0.0D;
    }
    ...
    public static EntityPlayer getClosestEnemy(double distance) {
        ...
        while (iterator.hasNext()) {
            ...
            if (!isntValid(player, distance)) {
                if (closest == null) {
                    closest = player;
                } else if (EntityUtil3.mc.player.getDistanceSq(player) < EntityUtil3.mc.player.getDistanceSq(closest)) {
                    closest = player;
                }
            }
        }
    ...
    public static boolean isInLiquid() {
        if (EntityUtil3.mc.player.fallDistance >= 3.0F) {
            ...
            AxisAlignedBB bb = EntityUtil3.mc.player.getRidingEntity() != null ? EntityUtil3.mc.player.getRidingEntity().getEntityBoundingBox() : EntityUtil3.mc.player.getEntityBoundingBox();
            int y = (int) bb.minY;

            for (int x = MathHelper.floor(bb.minX); x < MathHelper.floor(bb.maxX) + 1; ++x) {
                for (int z = MathHelper.floor(bb.minZ); z < MathHelper.floor(bb.maxZ) + 1; ++z) {
                    Block block = EntityUtil3.mc.world.getBlockState(new BlockPos(x, y, z)).getBlock();
    ...
    public static boolean isOnLiquid(double offset) {
        if (EntityUtil3.mc.player.fallDistance >= 3.0F) {
            ...
            AxisAlignedBB bb = EntityUtil3.mc.player.getRidingEntity() != null ? EntityUtil3.mc.player.getRidingEntity().getEntityBoundingBox().contract(0.0D, 0.0D, 0.0D).offset(0.0D, -offset, 0.0D) : EntityUtil3.mc.player.getEntityBoundingBox().contract(0.0D, 0.0D, 0.0D).offset(0.0D, -offset, 0.0D);
    ...
    public static boolean isOnLiquid() {
        double y = EntityUtil3.mc.player.posY - 0.03D;
    ...
    public static Map getTextRadarPlayers() {
        ...
        while (iterator.hasNext()) {
            ...
            if ((!player.isInvisible() || ((Boolean) Managers.getInstance().tRadarInv.getValue()).booleanValue()) && !player.getName().equals(EntityUtil3.mc.player.getName())) {
                ...
                healthSB.append(hp);
                int distanceInt = (int) EntityUtil3.mc.player.getDistance(player);
