...
public class BlockUtil3 implements Util {
    ...
    public static List getBlockSphere(float breakRange, Class clazz) {
        ...
        positions.addAll((Collection) BlockUtil.getSphere(EntityUtil.getPlayerPos(BlockUtil3.mc.player), breakRange, (int) breakRange, false, true, 0).stream().filter((pos) -> {
            return clazz.isInstance(BlockUtil3.mc.world.getBlockState(pos).getBlock());
    ...
    public static EnumFacing getFacing(BlockPos pos) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            RayTraceResult rayTraceResult = BlockUtil3.mc.world.rayTraceBlocks(new Vec3d(BlockUtil3.mc.player.posX, BlockUtil3.mc.player.posY + (double) BlockUtil3.mc.player.getEyeHeight(), BlockUtil3.mc.player.posZ), new Vec3d((double) pos.getX() + 0.5D + (double) facing.getDirectionVec().getX() * 1.0D / 2.0D, (double) pos.getY() + 0.5D + (double) facing.getDirectionVec().getY() * 1.0D / 2.0D, (double) pos.getZ() + 0.5D + (double) facing.getDirectionVec().getZ() * 1.0D / 2.0D), false, true, false);
        ...
        if ((double) pos.getY() > BlockUtil3.mc.player.posY + (double) BlockUtil3.mc.player.getEyeHeight()) {
    ...
    public static List getPossibleSides(BlockPos pos) {
        ...
        if (BlockUtil3.mc.world != null && pos != null) {
            ...
            for (int j = 0; j < i; ++j) {
                ...
                IBlockState blockState = BlockUtil3.mc.world.getBlockState(neighbour);
    ...
    public static EnumFacing getRayTraceFacing(BlockPos pos) {
        RayTraceResult result = BlockUtil3.mc.world.rayTraceBlocks(new Vec3d(BlockUtil3.mc.player.posX, BlockUtil3.mc.player.posY + (double) BlockUtil3.mc.player.getEyeHeight(), BlockUtil3.mc.player.posZ), new Vec3d((double) pos.getX() + 0.5D, (double) pos.getX() - 0.5D, (double) pos.getX() + 0.5D));
    ...
    public static int isPositionPlaceable(BlockPos pos, boolean rayTrace, boolean entityCheck) {
        Block block = BlockUtil3.mc.world.getBlockState(pos).getBlock();
        ...
        if (!(block instanceof BlockAir) && !(block instanceof BlockLiquid) && !(block instanceof BlockTallGrass) && !(block instanceof BlockFire) && !(block instanceof BlockDeadBush) && !(block instanceof BlockSnow)) {
            ...
            if (entityCheck) {
                iterator = BlockUtil3.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(pos)).iterator();
    ...
    public static void rightClickBlock(BlockPos pos, Vec3d vec, EnumHand hand, EnumFacing direction, boolean packet) {
        if (packet) {
            ...
            BlockUtil3.mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(pos, direction, hand, f, f1, f2));
        } else {
    ...
    public static void rightClickBed(BlockPos pos, float range, boolean rotate, EnumHand hand, AtomicDouble yaw, AtomicDouble pitch, AtomicBoolean rotating, boolean packet) {
        ...
        RayTraceResult result = BlockUtil3.mc.world.rayTraceBlocks(new Vec3d(BlockUtil3.mc.player.posX, BlockUtil3.mc.player.posY + (double) BlockUtil3.mc.player.getEyeHeight(), BlockUtil3.mc.player.posZ), posVec);
    ...
    public static void rightClickBlockLegit(BlockPos pos, float range, boolean rotate, EnumHand hand, AtomicDouble Yaw2, AtomicDouble Pitch, AtomicBoolean rotating, boolean packet) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (distanceSqHitVec <= MathUtil.square((double) range) && distanceSqHitVec < distanceSqPosVec && BlockUtil3.mc.world.rayTraceBlocks(eyesPos, hitVec, false, true, false) == null) {
                if (rotate) {
    ...
    public static boolean placeBlock(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean isSneaking) {
        ...
        if (side == null) {
            ...
            Block neighbourBlock = BlockUtil3.mc.world.getBlockState(neighbour).getBlock();
            ...
            if (!BlockUtil3.mc.player.isSneaking() && (BlockUtil3.blackList.contains(neighbourBlock) || BlockUtil3.shulkerList.contains(neighbourBlock))) {
                BlockUtil3.mc.player.connection.sendPacket(new CPacketEntityAction(BlockUtil3.mc.player, Action.START_SNEAKING));
    ...
    public static void placeBlockStopSneaking(BlockPos pos, EnumHand hand, boolean rotate, boolean packet, boolean isSneaking) {
        ...
        if (!isSneaking && sneaking) {
            BlockUtil3.mc.player.connection.sendPacket(new CPacketEntityAction(BlockUtil3.mc.player, Action.STOP_SNEAKING));
        }
    ...
    public static List possiblePlacePositions(float placeRange) {
        ...
        positions.addAll((Collection) BlockUtil.getSphere(EntityUtil.getPlayerPos(BlockUtil3.mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter(BlockUtil::canPlaceCrystal).collect(Collectors.toList()));
        return positions;
    }
    ...
    public static List possiblePlacePositions(float placeRange, boolean specialEntityCheck, boolean oneDot15) {
        ...
        positions.addAll((Collection) getSphere(EntityUtil.getPlayerPos(BlockUtil3.mc.player), placeRange, (int) placeRange, false, true, 0).stream().filter((pos) -> {
    ...
    public static boolean canPlaceCrystal(BlockPos blockPos, boolean specialEntityCheck, boolean oneDot15) {
        ...
        try {
            if (BlockUtil3.mc.world.getBlockState(blockPos).getBlock() != Blocks.BEDROCK && BlockUtil3.mc.world.getBlockState(blockPos).getBlock() != Blocks.OBSIDIAN) {
                return false;
            } else if ((oneDot15 || BlockUtil3.mc.world.getBlockState(boost2).getBlock() == Blocks.AIR) && BlockUtil3.mc.world.getBlockState(boost).getBlock() == Blocks.AIR) {
                Iterator ignored = BlockUtil3.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost)).iterator();
                ...
                do {
                    do {
                        if (!ignored.hasNext()) {
                            if (!oneDot15) {
                                ignored = BlockUtil3.mc.world.getEntitiesWithinAABB(Entity.class, new AxisAlignedBB(boost2)).iterator();
    ...
    public static Boolean isPosInFov(BlockPos pos) {
        ...
        return dirnumber == 0 && (double) pos.getZ() - BlockUtil3.mc.player.getPositionVector().z < 0.0D ? Boolean.valueOf(false) : (dirnumber == 1 && (double) pos.getX() - BlockUtil3.mc.player.getPositionVector().x > 0.0D ? Boolean.valueOf(false) : (dirnumber == 2 && (double) pos.getZ() - BlockUtil3.mc.player.getPositionVector().z > 0.0D ? Boolean.valueOf(false) : Boolean.valueOf(dirnumber != 3 || (double) pos.getX() - BlockUtil3.mc.player.getPositionVector().x >= 0.0D)));
    }
    ...
    public static boolean canBreak(BlockPos pos) {
        IBlockState blockState = BlockUtil3.mc.world.getBlockState(pos);
        Block block = blockState.getBlock();

        return block.getBlockHardness(blockState, BlockUtil3.mc.world, pos) != -1.0F;
    }
    ...
    public static boolean rayTracePlaceCheck(BlockPos pos, boolean shouldCheck, float height) {
        return !shouldCheck || BlockUtil3.mc.world.rayTraceBlocks(new Vec3d(BlockUtil3.mc.player.posX, BlockUtil3.mc.player.posY + (double) BlockUtil3.mc.player.getEyeHeight(), BlockUtil3.mc.player.posZ), new Vec3d((double) pos.getX(), (double) ((float) pos.getY() + height), (double) pos.getZ()), false, true, false) == null;
    }
    ...
    public static boolean isBlockValid(IBlockState blockState, BlockPos blockPos) {
        return blockState.getBlock() != Blocks.AIR ? false : (BlockUtil3.mc.player.getDistanceSq(blockPos) < 1.0D ? false : (BlockUtil3.mc.world.getBlockState(blockPos.up()).getBlock() != Blocks.AIR ? false : (BlockUtil3.mc.world.getBlockState(blockPos.up(2)).getBlock() != Blocks.AIR ? false : isBedrockHole(blockPos) || isObbyHole(blockPos) || isBothHole(blockPos) || isElseHole(blockPos))));
    }
    ...
    public static boolean isObbyHole(BlockPos blockPos) {
        ...
        for (int j = 0; j < i; ++j) {
            ...
            IBlockState touchingState = BlockUtil3.mc.world.getBlockState(pos);
