...
public final class GLUProjection {
    ...
    public void updateMatrices(IntBuffer viewport, FloatBuffer modelview, FloatBuffer projection, double widthScale, double heightScale) {
        ...
        this.fovY = fov = (float) Math.toDegrees(Math.atan(1.0D / (double) this.projection.get(5)) * 2.0D);
    ...
    public GLUProjection.Projection project(double x, double y, double z, GLUProjection.ClampMode clampModeOutside, boolean extrudeInverted) {
        if (this.viewport != null && this.modelview != null && this.projection != null) {
            ...
            if (outsideFrustum) {
                ...
                boolean[] guiY2 = this.doFrustumCheck(this.invFrustum, this.frustumPos, x, y, z);
                boolean guiX2 = guiY2[0] || guiY2[1] || guiY2[2] || guiY2[3];
                double guiX1;
                double guiY1;

                if ((!extrudeInverted || guiX2) && (!guiX2 || clampModeOutside == GLUProjection.ClampMode.NONE)) {
                    if (!GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection, this.viewport, this.coords)) {
                        ...
                        guiX1 = (double) this.coords.get(0) * this.widthScale;
                        guiY1 = (this.displayHeight - (double) this.coords.get(1)) * this.heightScale;
                        if (!opposite) {
                            return new GLUProjection.Projection(guiX1, guiY1, guiX2 ? GLUProjection.Projection.Type.OUTSIDE : GLUProjection.Projection.Type.INVERTED);
                        } else {
                            guiX1 = this.displayWidth * this.widthScale - guiX1;
                            guiY1 = this.displayHeight * this.heightScale - guiY1;
                            return new GLUProjection.Projection(guiX1, guiY1, guiX2 ? GLUProjection.Projection.Type.OUTSIDE : GLUProjection.Projection.Type.INVERTED);
                        }
                    }
                } else if ((!extrudeInverted || guiX2) && (clampModeOutside != GLUProjection.ClampMode.DIRECT || !guiX2)) {
                    if (clampModeOutside == GLUProjection.ClampMode.ORTHOGONAL && guiX2) {
                        if (!GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection, this.viewport, this.coords)) {
                            return new GLUProjection.Projection(0.0D, 0.0D, GLUProjection.Projection.Type.FAIL);
                        } else {
                            guiX1 = (double) this.coords.get(0) * this.widthScale;
                            ...
                            if (guiX1 < 0.0D) {
                                guiX1 = 0.0D;
                            } else if (guiX1 > this.displayWidth * this.widthScale) {
                                guiX1 = this.displayWidth * this.widthScale;
                            }

                            if (guiY1 < 0.0D) {
                                guiY1 = 0.0D;
                                return new GLUProjection.Projection(guiX1, guiY1, guiX2 ? GLUProjection.Projection.Type.OUTSIDE : GLUProjection.Projection.Type.INVERTED);
                            } else if (guiY1 <= this.displayHeight * this.heightScale) {
                                return new GLUProjection.Projection(guiX1, guiY1, guiX2 ? GLUProjection.Projection.Type.OUTSIDE : GLUProjection.Projection.Type.INVERTED);
                            } else {
                                guiY1 = this.displayHeight * this.heightScale;
                                return new GLUProjection.Projection(guiX1, guiY1, guiX2 ? GLUProjection.Projection.Type.OUTSIDE : GLUProjection.Projection.Type.INVERTED);
                            }
                        }
                    } else {
                    ...
                    if (!GLU.gluProject((float) x, (float) y, (float) z, this.modelview, this.projection, this.viewport, this.coords)) {
                        ...
                        if (opposite) {
                            guiX1 = this.displayWidth * this.widthScale - (double) this.coords.get(0) * this.widthScale - this.displayWidth * this.widthScale / 2.0D;
                            guiY1 = this.displayHeight * this.heightScale - (this.displayHeight - (double) this.coords.get(1)) * this.heightScale - this.displayHeight * this.heightScale / 2.0D;
                        } else {
                            guiX1 = (double) this.coords.get(0) * this.widthScale - this.displayWidth * this.widthScale / 2.0D;
                            guiY1 = (this.displayHeight - (double) this.coords.get(1)) * this.heightScale - this.displayHeight * this.heightScale / 2.0D;
                        }

                        GLUProjection.Vector3D vec = (new GLUProjection.Vector3D(guiX1, guiY1, 0.0D)).snormalize();

                        guiX1 = vec.x;
                        guiY1 = vec.y;
                        GLUProjection.Line vectorLine = new GLUProjection.Line(this.displayWidth * this.widthScale / 2.0D, this.displayHeight * this.heightScale / 2.0D, 0.0D, guiX1, guiY1, 0.0D);
                        ...
                        GLUProjection.Vector3D intersect = angle >= this.bra && angle < this.tra ? this.rb.intersect(vectorLine) : (angle >= this.tra && angle < this.tla ? this.tb.intersect(vectorLine) : (angle >= this.tla && angle < this.bla ? this.lb.intersect(vectorLine) : this.bb.intersect(vectorLine)));

                        return new GLUProjection.Projection(intersect.x, intersect.y, guiX2 ? GLUProjection.Projection.Type.OUTSIDE : GLUProjection.Projection.Type.INVERTED);
                    }
                }
    ...
    public static class Vector3D {
        ...
        public void setTitle(String defaultValue) {}
    }
