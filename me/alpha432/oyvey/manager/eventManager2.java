...
public class eventManager2 extends Feature {
    ...
    public void onUpdate(LivingUpdateEvent event) {
        if (!fullNullCheck() && event.getEntity().getEntityWorld().isRemote && event.getEntityLiving().equals(eventManager2.mc.player)) {
    ...
    public void onTick(ClientTickEvent event) {
        if (!fullNullCheck()) {
            ...
            Iterator iterator = eventManager2.mc.world.playerEntities.iterator();
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getStage() == 0) {
            ...
            if (event.getPacket() instanceof SPacketEntityStatus) {
                ...
                if (packet.getOpCode() == 35 && packet.getEntity(eventManager2.mc.world) instanceof EntityPlayer) {
                    EntityPlayer player = (EntityPlayer) packet.getEntity(eventManager2.mc.world);
    ...
    public void onRenderGameOverlayEvent(Text event) {
        if (event.getType().equals(ElementType.TEXT)) {
            ScaledResolution resolution = new ScaledResolution(eventManager2.mc);
    ...
    private static void lambda$onPacketReceive$1(SPacketPlayerListItem packet, AddPlayerData data) {
        ...
        case REMOVE_PLAYER:
            EntityPlayer entity = eventManager2.mc.world.getPlayerEntityByUUID(id);
