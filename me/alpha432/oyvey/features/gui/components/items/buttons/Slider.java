...
public class Slider extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, ((Number) this.setting.getValue()).floatValue() <= this.min.floatValue() ? this.x : this.x + ((float) this.width + 7.4F) * this.partialMultiplier(), this.y + (float) this.height - 0.5F, !this.isHovering(mouseX, mouseY) ? OyVey.colorManager.getColorWithAlpha(((Integer) ((ClickGui) OyVey.moduleManager.getModuleByClass(ClickGui.class)).hoverAlpha.getValue()).intValue()) : OyVey.colorManager.getColorWithAlpha(((Integer) ((ClickGui) OyVey.moduleManager.getModuleByClass(ClickGui.class)).alpha.getValue()).intValue()));
    ...
    private void setSettingFromX(int mouseX) {
        ...
        if (this.setting.getValue() instanceof Double) {
            ...
            this.setting.setValue(Double.valueOf((double) Math.round(10.0D * result) / 10.0D));
        } else if (this.setting.getValue() instanceof Float) {
            float result1 = ((Float) this.setting.getMin()).floatValue() + (float) this.difference * percent;

            this.setting.setValue(Float.valueOf((float) Math.round(10.0F * result1) / 10.0F));
        } else if (this.setting.getValue() instanceof Integer) {
            this.setting.setValue(Integer.valueOf(((Integer) this.setting.getMin()).intValue() + (int) ((float) this.difference * percent)));
        }
