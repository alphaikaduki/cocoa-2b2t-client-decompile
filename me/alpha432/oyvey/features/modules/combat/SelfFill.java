...
public class SelfFill extends Module {
    ...
    public void onUpdate() {
        ...
        if (SelfFill.mc.world.getBlockState(pos.down()).getBlock() == Blocks.AIR && BlockUtil.isPositionPlaceable(pos.down(), false) == 3) {
            BlockUtil.placeBlock(pos.down(), EnumHand.MAIN_HAND, false, ((Boolean) this.packet.getValue()).booleanValue(), false);
        }

        if (SelfFill.mc.world.getBlockState(pos.down()).getBlock() == Blocks.OBSIDIAN) {
            SelfFill.mc.player.connection.sendPacket(new Position(SelfFill.mc.player.posX, SelfFill.mc.player.posY - 1.3D, SelfFill.mc.player.posZ, false));
            SelfFill.mc.player.setPosition(SelfFill.mc.player.posX, SelfFill.mc.player.posY - 1.3D, SelfFill.mc.player.posZ);
            this.toggle();
        }
